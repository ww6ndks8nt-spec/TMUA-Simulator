...
(function(){
 ...
 const collectionOf=p=>p.esat?(p.exam==='ENGAA'?'official':'mat'):p.mat?'mat':p.stepAdapted?'step':p.group===4?'official':p.group===7?'community':p.group===3?'challenge':'practice';
 ...
 const categories=['official','practice','challenge','mat','step','community'];
 ...
 (ESAT_MODE?[['all','All papers'],['official','ENGAA'],['mat','NSAA']]:[['all','All collections'],['official','Official'],['practice','Practice sets'],['challenge','Challenge'],['mat','MAT'],['step','STEP'],['community','Community papers']]).forEach(([value,label])=>{const b=button(label,()=>{chosenCollection=value;filterPapers();},'du-collection-tab');b.dataset.collection=value;b.setAttribute('aria-pressed',String(value==='all'));tabHost.appendChild(b);});
 ...
 function refreshNext(){
  ...
  else host.appendChild(nextCard('Start a session','A fresh paper awaits',visiblePapers().length+(ESAT_MODE?' historical paper parts across ENGAA and NSAA.':' papers across six collections.')+' Choose the pace that works for you.','Browse papers',()=>safeNav(showLibraryHub)));
